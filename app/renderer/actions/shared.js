...
if (ipcRenderer) {
  ...
  ipcRenderer.on('appium-client-command-response', (evt, resp) => {
    const {id:elementId, variableName, variableType, source, screenshot, result, screenshotError, sourceError, uuid} = resp;
    let promise = clientMethodPromises[uuid];
    if (promise) {
      promise.resolve({elementId, variableName, variableType, source, screenshot, result, screenshotError, sourceError});
